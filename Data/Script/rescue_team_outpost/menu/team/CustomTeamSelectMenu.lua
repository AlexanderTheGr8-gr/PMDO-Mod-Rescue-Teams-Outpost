--[[
    CustomTeamSelectMenu
    Based on TeamSelectMenu by MistressNebula
...
CustomTeamSelectMenu = Class("CustomTeamSelectMenu")

--- Creates a new ``CustomTeamSelectMenu`` instance using the provided list and callbacks.
...
function CustomTeamSelectMenu:initialize(title, char_list, filter, confirm_action, refuse_action, use_submenu, menu_width, label)
...
function CustomTeamSelectMenu:createMenu()
...
function CustomTeamSelectMenu:load_chars(char_list)
    local list = {}
...
function CustomTeamSelectMenu:generate_options()
    local options = {}
...
function CustomTeamSelectMenu:choose(index)
...
function CustomTeamSelectMenu:callSubMenu(callback)
...
function CustomTeamSelectMenu:updateSummary()
...
TeamMultiSelectMenu = Class("TeamMultiSelectMenu", CustomTeamSelectMenu)
-------------------------------------------------------------------------------------------
...
function TeamMultiSelectMenu:initialize(title, char_list, filter, confirm_action, refuse_action, menu_width, label)
    ...
    CustomTeamSelectMenu.initialize(self, title, char_list, filter, confirm_action, refuse_action, menu_width, label)
end
...
--- @param parent table a ``CustomTeamSelectMenu``
...
-------------------------------------------------------------------------------------------
--- Creates a basic ``CustomTeamSelectMenu`` instance using the provided list and callbacks, then runs it and returns its output.
--- @param title string the title this window will have
--- @param char_list table an array, list or lua array table containing ``RogueEssence.Dungeon.Character`` objects.
--- @param filter function a function that takes a ``RogueEssence.Dungeon.Character`` object and returns a boolean. Any character that does not pass this check will have its option disabled in the menu. Defaults to ``return true``.
--- @param use_submenu boolean whether or not to call the ``TeamSelectSubMenu`` before returning. Defaults to true.
--- @return userdata the selected character if one was chosen in the menu; ``nil`` otherwise.
function CustomTeamSelectMenu.run(title, char_list, filter, use_submenu)
    local ret
    ...
    local menu = CustomTeamSelectMenu:new(title, char_list, filter, choose, refuse, use_submenu)
    UI:SetCustomMenu(menu.menu)
    UI:WaitForChoice()
    return ret
end

--- Creates a ``CustomTeamSelectMenu`` instance that allows a choice between the current party members.
--- @param filter function a function that takes a ``RogueEssence.Dungeon.Character`` object and returns a boolean. Any character that does not pass this check will have its option disabled in the menu. Defaults to ``return true``.
--- @param use_submenu boolean whether or not to call the ``TeamSelectSubMenu`` before returning. Defaults to true.
--- @return userdata the selected character if one was chosen in the menu; ``nil`` otherwise.
function CustomTeamSelectMenu.runPartyMenu(filter, use_submenu)
    local char_list = _DATA.Save.ActiveTeam.Players

    return CustomTeamSelectMenu.run(STRINGS:FormatKey("MENU_TEAM_TITLE"), char_list, filter, use_submenu)
end
