--[[
    CustomAssemblySelectMenu
    Based on AssemblySelectMenu by MistressNebula
...
CustomAssemblySelectMenu = Class("CustomAssemblySelectMenu", TeamSelectMenu)

--- Creates a new ``CustomAssemblySelectMenu`` instance using the provided list and callbacks.
...
--- @param use_submenu boolean whether or not to call the ``CustomAssemblySelectSubMenu`` before returning. Defaults to true.
...
function CustomAssemblySelectMenu:initialize(filter, confirm_action, refuse_action, use_submenu, menu_width, sort_mode, label)
    ...
    if not self.mode_to_int[self.sort_mode] then
        PrintInfo("sort_mode parameter for CustomAssemblySelectMenu was not a valid AssemblySortMode. It will default to \"Recent\"")
...
function CustomAssemblySelectMenu:createMenu()
    self.MAX_ELEMENTS = 6
...
function CustomAssemblySelectMenu:createPortraitBox()
...
function CustomAssemblySelectMenu:load_chars(_)
    local list = {}
...
function CustomAssemblySelectMenu:getSortedAssembly()
...
function CustomAssemblySelectMenu:assemblyCompare(a, b)
...
function CustomAssemblySelectMenu:generate_options()
    local options = {}
    for i=1, #self.charList, 1 do
        ...
        if enabled ~= nil then
            ...
            local option = RogueEssence.Menu.MenuElementChoice(function() self:choose(i) end, (enabled and {true} or {false})[1], text_name, text_lv_label, text_level)
            if enabled == 1 then option:SilentSelect(true) end
...
function CustomAssemblySelectMenu:callSubMenu(callback)
    _MENU:AddMenu(CustomAssemblySelectSubMenu:new(self, function() callback(true) end, function() callback(false) end).menu, true)
end

--- Uses the current input to apply changes to the menu.
--- @param input userdata the ``RogueEssense.InputManager``.
function CustomAssemblySelectMenu:updateFunction(input)
...
--- @return table an ``CustomAssemblySelectMenu``.
function CustomAssemblySelectMenu:cloneMenu(new_mode)
    return CustomAssemblySelectMenu:new(self.filter, self.confirmAction, self.refuseAction, self.use_submenu, self.menuWidth, new_mode)
end

--- Updates the portrait and the summary window.
function CustomAssemblySelectMenu:updateSummary()
...
CustomAssemblyMultiSelectMenu = Class("CustomAssemblyMultiSelectMenu", CustomAssemblySelectMenu)
-------------------------------------------------------------------------------------------

--- Creates a new ``CustomAssemblyMultiSelectMenu`` instance using the provided list and callbacks.
--- @param filter function a function that takes a ``RogueEssence.Dungeon.Character`` object and returns a boolean. Any character that does not pass this check will have its option disabled in the menu. Defaults to ``return true``.
--- @param confirm_action function the function called when a slot is chosen. It will have a ``RogueEssence.Dungeon.Character`` passed to it as a parameter.
--- @param refuse_action function the function called when the player presses the cancel or menu button.
--- @param use_submenu boolean whether or not to call the ``CustomAssemblySelectSubMenu`` before returning. Defaults to true.
...
--- @param sort_mode userdata a ``RogueEssence.Menu.AssemblyMenu.AssemblySortMode`` object. defaults to ``Recent``
--- @param label string the label that will be applied to this menu. Defaults to "ASSEMBLY_MENU_LUA"
function CustomAssemblyMultiSelectMenu:initialize(filter, confirm_action, refuse_action, use_submenu, menu_width, sort_mode, label, max_select)
    self.max_select = (max_select ~= nil) and max_select or nil
    self.multiConfirmAction = function(list)
        self.choice = self:multiConfirm(list)
        self.confirmAction(self.choice)
    end
    CustomAssemblySelectMenu.initialize(self, filter, confirm_action, refuse_action, use_submenu, menu_width, sort_mode, label)
end

--- Performs the final adjustments and creates the actual menu object.
function CustomAssemblyMultiSelectMenu:createMenu()
    self.MAX_ELEMENTS = 6

    -- local valid = self:count_valid()
    local origin = RogueElements.Loc(16,16)
    local option_array = luanet.make_array(RogueEssence.Menu.MenuElementChoice, self.optionsList)
    self.menu = RogueEssence.Menu.ScriptableMultiPageMenu(self.label, origin, self.menuWidth, self.title, option_array, 0, self.MAX_ELEMENTS, self.refuseAction, self.refuseAction, true, self.max_select, self.multiConfirmAction)
    self.menu.ChoiceChangedFunction = function() self:updateSummary() end
end

--- Counts the number of valid options generated.
--- @return number the number of valid options.
function CustomAssemblyMultiSelectMenu:count_valid()
    local count = 0
...
function CustomAssemblyMultiSelectMenu:choose(index)
...
function CustomAssemblyMultiSelectMenu:multiConfirm(list)
    local result = {}
...
--- Returns a newly created copy of this object with the provided ``RogueEssence.Menu.AssemblyMenu.AssemblySortMode``
--- @param new_mode userdata the ``RogueEssence.Menu.AssemblyMenu.AssemblySortMode`` to be applied
--- @return table an ``CustomAssemblyMultiSelectMenu``.
function CustomAssemblyMultiSelectMenu:cloneMenu(new_mode)
    local selected_team, selected = self:saveSelectedMembers()
    local new_menu = CustomAssemblyMultiSelectMenu:new(self.filter, self.confirmAction, self.refuseAction, self.use_submenu, self.menuWidth, new_mode)
...
function CustomAssemblyMultiSelectMenu:saveSelectedMembers()
    local selected = {}
    local selected_team = {}
...
function CustomAssemblyMultiSelectMenu:loadSelectedMembers(selected_team, selected)
    local num = _DATA.Save.ActiveTeam.Players.Count
...
CustomAssemblySelectSubMenu = Class("CustomAssemblySelectSubMenu", TeamSelectSubMenu)
-------------------------------------------------------------------------------------------

--- Creates a new ``CustomAssemblySelectSubMenu`` instance using the provided data and callbacks.
--- @param parent table an ``CustomAssemblySelectMenu``
...
function CustomAssemblySelectSubMenu:initialize(parent, confirm_action, refuse_action, label)
...
function CustomAssemblySelectSubMenu:openSummary()
...
function CustomAssemblySelectMenu.run(filter, use_submenu)
    local ret
    ...
    local menu = CustomAssemblySelectMenu:new(filter, choose, refuse, use_submenu)
...
--- @param filter function a function that takes a ``RogueEssence.Dungeon.Character`` object and returns a boolean. Any character that does not pass this check will have its option disabled in the menu. Defaults to ``return true``.
--- @param use_submenu boolean whether or not to call the ``CustomAssemblySelectSubMenu`` before returning. Defaults to true. Only appears if a character is chosen without selecting.
--- @param max_select boolean Maximum number of pokemons to select.
--- @return table the list of selected characters if at least one was chosen in the menu; ``nil`` otherwise.
function CustomAssemblyMultiSelectMenu.runMultiMenu(filter, use_submenu, max_select)
    local ret = {}
    ...
    local refuse = function() _MENU:RemoveMenu() end
    local menu = CustomAssemblyMultiSelectMenu:new(filter, choose, refuse, use_submenu, nil, nil, nil, max_select)
    UI:SetCustomMenu(menu.menu)
    UI:WaitForChoice()
    return ret
end
